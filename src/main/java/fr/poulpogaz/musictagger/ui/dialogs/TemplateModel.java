...
import fr.poulpogaz.musictagger.model.Key;
...
public class TemplateModel {
    ...
    public void assertValid() {
        ...
        if (nameError != null) {
            throw new MTException(nameError);
        }

        String keyError = checkKeys();
        if (keyError != null) {
            throw new MTException(keyError);
        }
    }
    ...
    private void createNewTemplate() {
        ...
        for (int i = 0; i < keyTable.getRowCount(); i++) {
            ...
            if (!template.addKey(key.asKey())) {
                throw new MTException("Cannot create template: key " + key + " wasn't added to template " + template);
            }
        }
